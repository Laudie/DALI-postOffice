...
:-write('Sono il cliente 6').

:-dynamic biglietto/2.

eve(sveglia(var_Id,var_Biglietto)):-assert(biglietto(var_Id,var_Biglietto)),once(a(messageOperatore)).

a(messageOperatore):-biglietto(var_Id,var_Biglietto),a(message(agentOperazioniVarie,send_message(chiedo_di_fare_operazione(var_Id,var_Biglietto),var_Me))).

eve(fai_operazione):-write('sto ritirando un pacco'),nl,a(operazione_terminata).

a(operazione_terminata):-write('ho terminato'),nl,biglietto(var_Id,var_Biglietto),a(message(agentOperazioniVarie,send_message(termino_operazione(var_Id,var_Biglietto),var_Me))),retract(biglietto(var_Id,var_Biglietto)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_37637,var_Ontology,_37641),_37631),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_37675)),a(message(var_Ag,propose(var_A,[_37675],var_AgI))),retractall(ext_agent(var_Ag,_37713,var_Ontology,_37717)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_37511,var_Ontology,_37515),_37505),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_37581,var_Ontology,_37585)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_37399,var_Ontology,_37403),_37393),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_37455,var_Ontology,_37459)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_36963),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_36997),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_36779).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_36627),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_36575),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_36451,_36453)),clause(agente(_36473,_36475,_36477,var_S),_36469),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_36245,_36247,_36249,var_S),_36241),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_36345,[]),repeat,read(_36345,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_36345).
