...
:-write('Sono il cliente 3').

:-dynamic biglietto/2.

eve(sveglia(var_Id,var_Biglietto)):-assert(biglietto(var_Id,var_Biglietto)),once(a(messageOperatore)).

a(messageOperatore):-biglietto(var_Id,var_Biglietto),a(message(agentSpedizione,send_message(chiedo_di_fare_operazione(var_Id,var_Biglietto),var_Me))).

eve(fai_operazione):-write('sto ritirando un pacco'),nl,a(operazione_terminata).

a(operazione_terminata):-write('ho terminato'),nl,biglietto(var_Id,var_Biglietto),a(message(agentSpedizione,send_message(termino_operazione(var_Id,var_Biglietto),var_Me))),retract(biglietto(var_Id,var_Biglietto)).
