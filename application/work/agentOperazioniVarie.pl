...
:-write('Sono operatore per le operazioni varie').

:-assert(operatore_libero(true)).

eve(chiedo_di_fare_operazione(var_Id,var_Biglietto)):-now(var_Timestamp),assert(cliente_prenotato(var_Id,var_Biglietto,var_Timestamp)).

operatore_libero:-write('').

evi(operatore_libero):-operatore_libero(var_X),true==var_X,cliente_prenotato(var_Id,var_Biglietto,var_Timestamp),a(elabora_fila).

a(elabora_fila):-write('Operatore sta gestendo la fila...'),nl,cliente_prenotato(var__,var_Xp,var__),not((cliente_prenotato(var__,var_Xp2,var__),var_Xp2<var_Xp)),cliente_prenotato(var_Id,var_Xp,var_Xt),not((cliente_prenotato(var__,var_Xp,var_Xt2),var_Xt2<var_Xt)),format('Cliente successivo: ~d',var_Id),nl,retract(operatore_libero(true)),assertz(operatore_libero(false)),a(concedi_operazione(var_Id)).

a(concedi_operazione(var_Id)):-format('Ora il cliente con id ~d puo passare',var_Id),nl,number_chars(var_Id,var_App),atom_chars(var_IdString,var_App),atom_concat('agentCliente',var_IdString,var_AgentName),a(message(var_AgentName,send_message(fai_operazione,var_Me))).

eve(termino_operazione(var_Id,var_Biglietto)):-retract(cliente_prenotato(var_Id,var__,var__)),retract(operatore_libero(false)),assertz(operatore_libero(true)),write('operatore libero').
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_38985,var_Ontology,_38989),_38979),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_39023)),a(message(var_Ag,propose(var_A,[_39023],var_AgI))),retractall(ext_agent(var_Ag,_39061,var_Ontology,_39065)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_38859,var_Ontology,_38863),_38853),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_38929,var_Ontology,_38933)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_38747,var_Ontology,_38751),_38741),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_38803,var_Ontology,_38807)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_38311),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_38345),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_38127).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_37975),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_37923),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_37799,_37801)),clause(agente(_37821,_37823,_37825,var_S),_37817),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_37593,_37595,_37597,var_S),_37589),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_37693,[]),repeat,read(_37693,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_37693).
